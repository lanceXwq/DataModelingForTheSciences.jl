...
md"""
## Method 1: Sample from multivariate normal
"""

# ╔═╡ 6f96d2f3-7223-4f49-ba2f-e9c30427cc48
μ = @. (x_T - x_0) / t[end] * t + x_0

# ╔═╡ 2f9c9ba9-a9d8-45e1-b415-bec228f0607a
function getcovariancematrix(D, t)
    tᵀ = transpose(t)
    C = (t .+ tᵀ .- abs.(t .- tᵀ)) ./ 2 .- t .* tᵀ ./ t[end]
    C .*= 2 * D
    return PSDMat(C)
end

# ╔═╡ 4f8eecaa-9e78-401b-811a-6a05e5fd281e
C = getcovariancematrix(D, t)

# ╔═╡ 30bae926-b198-475d-b59b-3ed7c6ec01be
distr = MvNormal(μ, C);

# ╔═╡ 60774527-792c-459e-ab0e-0e07fde8b7b4
x = rand(distr, 10);

# ╔═╡ ad5d5ba2-3afb-4dbc-9dee-0c8d2df70e0d
begin
    fig, ax, lins = series(t, transpose(x), color=:tab10)
    lines!(ax, t, μ, color=:black, linestyle=:dash)
    fig
end

# ╔═╡ 9df1c09b-27e8-4619-8dee-9301a76aaf1b
md"""
## Method 2: Map from Brownian trajectories
"""

# ╔═╡ 3938f0d9-bc0e-4247-af8a-924159244c36
Δy = sqrt(2 * D * dt) .* randn(length(t), 10);

# ╔═╡ 29309e10-c001-4816-b339-885e29193a40
Δy[1, :] .= x_0 - x_T;
...
@views @. y -= y[end:end, :] * t / t[end] - x_T

# ╔═╡ 98e53e51-6890-4cc4-a182-f85c99fa9cba
begin
    fig2, ax2, lins2 = series(t, transpose(y), color=:tab10)
    lines!(ax2, t, μ, color=:black, linestyle=:dash)
    fig2
end
...
[[deps.PDMatsExtras]]
deps = ["ChainRulesCore", "LinearAlgebra", "PDMats", "SparseArrays", "SuiteSparse"]
git-tree-sha1 = "a7f248b88bc83244468e7172969a2a6b8cc2fd80"
