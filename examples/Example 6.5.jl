...
# ╔═╡ 13053f70-efbd-4005-a39c-72f4368b36c7
begin
    using Distributions
    md"""
    !!! info "Packages"
...
# ╔═╡ f31ce8fc-6d6a-488f-8b8b-df78952116e5
begin
    using GLMakie
    md"""
    !!! info "Packages"
...
# ╔═╡ eaaf1f46-ee73-11ee-1786-43ca16e82e40
md"""
# Example 6.5: BetaBernP in a regression problem
"""

# ╔═╡ f6188235-7d19-4353-84b6-47e01332dfbc
M, N = 5, 10;

# ╔═╡ 7607ee32-64bf-433c-bdd6-51b285f418d0
md"""
Assuming there are at most ``M`` = $M states and ``N`` = $N data points.
"""

# ╔═╡ 03c500a1-4cbe-4066-b584-539a7898a0fc
α, β = 1, 1;

# ╔═╡ 9018075e-ac15-43a4-9485-14588f5de5be
md"""
Parameters for π's prior are ``\alpha`` = $α and ``\beta`` = $β.
"""

# ╔═╡ 9541e662-aeac-4c06-8b9b-3939a4306ed3
μ_mol = 3;

# ╔═╡ edc0cdbf-b02f-4b04-a06e-409ebadaceae
md"""
Molecular brightness ``μ\_mol`` = $μ_mol.
"""

# ╔═╡ 5f4c580a-1981-4306-aa07-2a6693dbc9bd
w = rand(Poisson(μ_mol * 3), N)

# ╔═╡ 1fd140f5-ada5-445a-99c4-eae4bd56bbfd
sample_π(α, β, M, b) = rand.(Beta.(α / M, β * (M - 1) / M .+ .~b))

# ╔═╡ c8d16e21-2ce4-4115-8811-a4101bcda187
propose_b(π) = rand(length(π)) .< π

# ╔═╡ a707d7b4-1f3a-48c1-8ee3-3e41809fa865
function get_Δloglikelihood(Σw, b_prop, b_old, N, μ_mol)
    B_prop, B_old = count(b_prop), count(b_old)
    return Σw * (log(B_prop) - log(B_old)) + N * μ_mol * (B_old - B_prop)
end

# ╔═╡ 8902f987-c5ce-4c83-aa59-15344148834c
b = rand(M) .< 0.5

# ╔═╡ ed5aa3f8-9eae-4f00-a449-ee20261d4825
π = zeros(M) .+ 0.5

# ╔═╡ c3f80cf4-fbfb-4a3c-b0b3-692d599bf898
Σw = sum(w)

# ╔═╡ 736e1788-566f-425f-9f13-a3b7fa582f10
B = Vector{Int}(undef, 100);

# ╔═╡ 4190c909-de6c-4e08-9bcd-61b1f7e4671f
for i in 1:length(B)
    π .= sample_π(α, β, M, b)
    b_prop = propose_b(π)
    log_A = get_Δloglikelihood(Σw, b_prop, b, N, μ_mol)
    if log(rand()) <= log_A
        b .= b_prop
    end
    B[i] = count(b)
...
# ╔═╡ 7d1318ef-10e2-4279-9642-c1d3d9f98e15
lines(B)
...
manifest_format = "2.0"
project_hash = "c008738e119364b23796bbde1cc915b5cc6531ee"
