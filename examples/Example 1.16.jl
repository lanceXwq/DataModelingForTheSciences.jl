...
# ╔═╡ 30cfe921-bdc7-42bb-af4e-cf58f0599289
begin
    using GLMakie
    md"""
...
# ╔═╡ d45f6904-effb-11ee-279d-b309b9e35f08
md"""
# Example 1.16: Likelihood based modeling and inference
"""

# ╔═╡ 54787e2a-cd10-4c34-95ac-c7f28b768232
N = 100;

# ╔═╡ 2088a9a6-092a-4c99-b872-0db250c55963
π = 0.3;

# ╔═╡ 0ae065ae-db9c-43fd-9cce-a24d6a37b9be
w = rand(N) .< π;

# ╔═╡ 207ecd2e-8541-4e7e-96b4-959223119504
M = count(w);

# ╔═╡ cf947aa9-fa29-4e35-90c4-d75a171cb90e
get_likelihood(π, M, N) = @. π^M * (1 - π)^(N - M)

# ╔═╡ b0ee70e7-492d-4c93-b02a-ebc7b5b0cae3
begin
    likelihood = get_likelihood(0.01:0.01:0.99, M, N)
    fig, ax, lins = lines(0.01:0.01:0.99, likelihood)
    vlines!(ax, π, linestyle=:dash)
    vlines!(ax, M / N, linestyle=:dash)
    ylims!(ax, 0, nothing)
    fig
end

# ╔═╡ 0729cfab-f721-422e-ac36-c1df54cc240e
get_loglikelihood(π, M, N) = @. M * log(π) + (N - M) * log(1 - π)

# ╔═╡ 26cebb4e-4e74-43d7-816d-7265715be9a4
begin
    loglikelihood = get_loglikelihood(0.01:0.01:0.99, M, N)
    fig2, ax2, lins2 = lines(0.01:0.01:0.99, loglikelihood)
    vlines!(ax2, π, linestyle=:dash)
    vlines!(ax2, M / N, linestyle=:dash)
    fig2
end
...
manifest_format = "2.0"
project_hash = "a41557fbdba021955b162b527d9058c555e786ad"
